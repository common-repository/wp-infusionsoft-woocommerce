<?php
/*
* Plugin Name: WP Keap/Infusionsoft WooCommerce
* Description: Integrates WooCommerce with Keap/Infusionsoft allowing new orders to be automatically sent to your Keap/Infusionsoft account.
* Version: 1.1.5
* Requires at least: 3.8
* Author: CRM Perks
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/woocommerce-plugins/woocommerce-infusionsoft-plugin/
* Text Domain: wp-infusionsoft-woocommerce
...
if( !class_exists( 'vxc_infusionsoft' ) ):
...
class vxc_infusionsoft{            
 public $url='https://www.crmperks.com';           
  ...
  private $temp= '';
  public static $note= '';
  public static $title='WooCommerce Infusionsoft Plugin';  
...
include_once($pro_file);
  ...
  add_action('woocommerce_saved_order_items',array($this,'save_lines'),999,2); //update order items 
   ...
   //save user 
add_action( 'profile_update',array($this,'profile_update_set'), 990 );
...
add_action('woocommerce_save_product_variation',array($this,'save_variation'),999);  */ 
  ...
  if(isset($_GET['affiliate'])){
     $aff_id=$this->post('affiliate');
  }else  if(isset($_GET['affid'])){
     $aff_id=$this->post('affid');
  }
  
  if(!empty($aff_id)){
      setcookie('vxc_aff_id', $aff_id, time()+25920000, '/');
  }
  ...
  load_plugin_textdomain('wp-infusionsoft-woocommerce', FALSE, $this->plugin_dir_name() . '/languages/' );
  ...
  $log_str="Installing WooCommerce Infusionsoft Plugin version=".$this->version;
...
*/
  ...
  $message = sprintf(__("%sWooCommerce%s is required. %sDownload latest version!%s", 'wp-infusionsoft-woocommerce'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('WooCommerce is installed but not active. %sActivate WooCommerce%s to use the WooCommerce Infusionsoft Plugin','wp-infusionsoft-woocommerce'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=woocommerce/woocommerce.php'), 'activate-plugin_woocommerce/woocommerce.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sWooCommerce%s is required. %sDownload latest version!%s", 'wp-infusionsoft-woocommerce'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");
  }  
 ...
 $this->push($id,'update_order');
  ...
  $this->push($id,$status);
    ...
    public function order_submit_new($id){ 
              if(!empty($_COOKIE['vxc_aff_id'])){
      $meta_value=sanitize_text_field($_COOKIE['vxc_aff_id']);
   update_post_meta( $id, 'vxc_affiliate_id', $meta_value );       
  }
      if($this->do_actions()){ 
do_action('vx_addons_save_entry',$id,'','wc','');         
  ...
  */
  ...
  return "<div class='alert_danger crm_alert'>".sprintf(__("Please Configure %sInfusionsoft Settings%s",'wp-infusionsoft-woocommerce'),"<a href='".$link."'>","</a>")."</div>";
...
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','wp-infusionsoft-woocommerce') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
...
return  $var;
  ...
  */
  ...
  if($note['status'] == "4"){
    $msg=sprintf(__('Infusionsoft (%s) Filtered','wp-infusionsoft-woocommerce'),$object);    
  }else{
      //web2lead do not have link
  $link=sprintf(__("with ID # %s",'wp-infusionsoft-woocommerce'),$this->post('id',$note));
  if($this->post('link',$note) !=""){
      $id_link='<a href="'.$this->post('link',$note).'" target="_blank" title="'.$this->post('id',$note).'">'.$this->post('id',$note).'</a>';
  $link=sprintf(__('with ID # %s','wp-infusionsoft-woocommerce'),$id_link);
  }
...
if($note['status'] == '1'){
  $msg=sprintf(__('Added to Infusionsoft (%s) ','wp-infusionsoft-woocommerce'),$object).$link;
  }else{
  $msg=sprintf(__("Updated to Infusionsoft (%s) ",'wp-infusionsoft-woocommerce'),$object).$link;     
  }                                         
  }
  }else if($show_error){
     if(empty($error)){
  ...
  $msg.=' - <a href="'.esc_url($log_url).'" class="vx_log_link" title="'.__('View Detail','wp-infusionsoft-woocommerce').'" data-id="'.esc_html($note['log_id']).'">'.__('View Detail','wp-infusionsoft-woocommerce')."</a>";
  } 
  return $msg;
  }
...
return $results;
  ...
  /**
  * Push order to infusionsoft feeds
  *     
  * @param mixed $order_id
  * @param mixed $status
  */
...
if($value === ''){
 ...
 //var_dump($temp);die();
  ...
  if(isset($feed['owner']) && $feed['owner'] == "1"){
   $temp['OwnerID']=array('value'=> apply_filters('vx_assigned_user_id',$feed['owner'],$this->id,$feed['id'],$order,$_order),'label'=>'Owner ID');   
  }

 $aff_id=get_post_meta( $order_id, 'vxc_affiliate_id', true);
if(!empty($aff_id)){
  $temp['Lead_Affiliate_Id']=array('value'=>$aff_id,'label'=>'Affiliate ID');  
    if($object == 'Order'){
   $temp['Sales_Affiliate_Id']=array('value'=>$aff_id,'label'=>'Sales Affiliate ID');      
    }
}
  ...
  //
    ...
    //apply tags
  if(!empty($feed['tag_check']) && !empty($feed['tag_ids'])){
       $tags=is_array($feed['tag_ids']) ? $feed['tag_ids'] : array();
        $tag_res=array();
        foreach($tags as $k=>$tag){ 
            if(empty($tag) || !is_array($tag)){ continue; }
          $filters=!empty($feed['tags'][$k]) ? array($feed['tags'][$k]) : array(); 
        
        if( $this->check_filter($filters)){     
  ...
  $no_filter=$this->check_filter($filters);
...
//  if($this->post('disable_log',$settings) !="yes"){
  ...
  */
  ...
  $subject=__('Error While Posting to Infusionsoft','wp-infusionsoft-woocommerce');
$page_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; 
  ...
  $email_info=array("msg"=>$res['error'],"title"=>__("Infusionsoft Error",'wp-infusionsoft-woocommerce'),"info_title"=>"More Detail","info"=>array("Order Id"=>$order_url,"Time"=>date('d/M/y H:i:s',current_time('timestamp')),"Page URL"=>'<a href="'.esc_url($page_url).'" style="word-break:break-all;">'.esc_url($page_url).'</a>'));
  ...
  */
  ...
  $status=0;
  if(class_exists('vxc_infusionsofts_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/wp-infusionsoft-woocommerce/wp-infusionsoft-woocommerce.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function get_booking_ids_from_order_item_id( $order_item_id ) {
  ...
  /**
  * Initialize infusionsoft api
  *  
  * @param mixed $crm
  * @return VXInfusionsoftAPI
  */
...
return $result;
  ...
  */
  public function get_objects($api_type="",$info="",$refresh=false){
      ...
      $objects=array("Contact"=>'Contact',"Company"=>'Company','Order'=>'Order');
...
$vxc_infusionsoft=new vxc_infusionsoft(); 
$vxc_infusionsoft->instance();
$vx_wc['vxc_infusionsoft']='vxc_infusionsoft';

...
display: inline; 
  ...
  jQuery(document).ready(function($){
    ...
    if(unsaved) return '<?php esc_html_e('Changes you made may not be saved','wp-infusionsoft-woocommerce'); ?>';
});
          // Tooltips
  var tiptip_args = {
  'attribute' : 'data-tip',
  ...
  $(this).text('<?php esc_html_e('Show Key','wp-infusionsoft-woocommerce') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php esc_html_e('Hide Key','wp-infusionsoft-woocommerce') ?>');  

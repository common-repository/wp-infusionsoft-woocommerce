...
if( !class_exists( 'vxc_pages_infusionsoft' ) ) {

/**
* @since       1.0.0
*/
class vxc_pages_infusionsoft   extends vxc_infusionsoft{  
...
if($post_type == 'shop_order'){
  ...
  */
  ...
  $msg=__('You do not have permissions to export logs','wp-infusionsoft-woocommerce');
 ...
 $sql="select * $sql_end limit 3000";
  ...
  $fields=array(); $field_titles=array(__("SNO",'wp-infusionsoft-woocommerce'),__("Status",'wp-infusionsoft-woocommerce'),__("Infusionsoft ID",'wp-infusionsoft-woocommerce') ,__("Order ID",'wp-infusionsoft-woocommerce'),__("Description",'wp-infusionsoft-woocommerce'),__("Infusionsoft Link",'wp-infusionsoft-woocommerce'),__("Time",'wp-infusionsoft-woocommerce'));
  ...
  $msg=__('You do not have permissions to clear logs','wp-infusionsoft-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
  global $wpdb;
  ...
  */
  ...
  if(isset($_GET[$this->id."_logs"])){
      $msg=__('Error While Clearing Infusionsoft Logs','wp-infusionsoft-woocommerce');
      $level="error";
      if(!empty($_GET[$this->id."_logs"])){
      $msg=__('Infusionsoft Logs Cleared Successfully','wp-infusionsoft-woocommerce');   
      $level="updated";
      }
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'wp-infusionsoft-woocommerce') . '</a>' );
        }
        return $links;
    } 
    /**
  ...
  */
  ...
  $sections=array(""=>__('Infusionsoft Settings','wp-infusionsoft-woocommerce'),'vxc_uninstall'=>__('Uninstall','wp-infusionsoft-woocommerce'));
  ...
  */
  ...
  $settings_tabs[$this->id] = __( 'Infusionsoft', 'wp-infusionsoft-woocommerce' );
        }
  ...
  */
  ...
  add_submenu_page( 'woocommerce', __( 'Infusionsoft Log','wp-infusionsoft-woocommerce'),  __( 'Infusionsoft Log','wp-infusionsoft-woocommerce') , $this->id."_read_logs", $this->id.'_log', array( $this, 'log_page' ) );
  }
 ...
 include_once(self::$path . "includes/install.php"); 
  do_action('uninstall_vx_plugin_'.$this->type);
   $install=new vxc_install_infusionsoft();
  $install->remove_data();
  ...
  */
  ...
  array(
  'labels' => array(
  'name'                  => __( 'Infusionsoft', 'wp-infusionsoft-woocommerce' ),
  'singular_name'         => __( 'Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'menu_name'             => _x( 'Infusionsoft Feeds', 'Admin menu name', 'wp-infusionsoft-woocommerce' ),
  'add_new'               => __( 'New Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'add_new_item'          => __( 'Add New Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'edit'                  => __( 'Edit', 'wp-infusionsoft-woocommerce' ),
  'edit_item'             => __( 'Edit Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'new_item'              => __( 'New Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'view'                  => __( 'View Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'view_item'             => __( 'View Infusionsoft Feed', 'wp-infusionsoft-woocommerce' ),
  'search_items'          => __( 'Search Infusionsoft Feeds', 'wp-infusionsoft-woocommerce' ),
  'not_found'             => __( 'No Infusionsoft Feeds found', 'wp-infusionsoft-woocommerce' ),
  'not_found_in_trash'    => __( 'No Infusionsoft Feeds found in trash', 'wp-infusionsoft-woocommerce' ),
  'parent'                => __( 'Parent Infusionsoft Feed', 'wp-infusionsoft-woocommerce' )
  ),
  'description'         => __( 'This is where feeds are stored.', 'wp-infusionsoft-woocommerce' ),
  'public'              => false,
  'show_ui'             => true,
  'capability_type'     => $this->id,
  //  'capabilities'=>array('read_vxc_infusionsoft1s'),
  'map_meta_cap'        => true,
  ...
  */
  ...
  1 => __( 'Infusionsoft Feed updated.', 'wp-infusionsoft-woocommerce' ),
  6 => __( 'Infusionsoft Feed updated.', 'wp-infusionsoft-woocommerce' )
  );
  return $messages;
  } 
  /**
  ...
  */
  ...
  add_meta_box(
  $this->id."_crm_box", //$id
  __( 'Infusionsoft', 'wp-infusionsoft-woocommerce' ), //$title
  ...
  */
  ...
  __( 'Infusionsoft Fields Mapping', 'wp-infusionsoft-woocommerce' ), //$title
...
if(!empty($v['placeholder'])){ $label=$v['placeholder']; }
  ...
  '_order_tax_total_tax_ship'=>array('label'=>'Order Tax Total + Shipping Total'),
...
$item_fields=array('__vxp_iun-name'=>array('label'=>'Item Name'),'__vxp_iun-type'=>array('label'=>'Item Type'),'__vxp_iun-id'=>array('label'=>'Item ID'));
  ...
  * @param mixed $crm
  */
  private function get_field_mapping($feed,$info="",$object=""){ ///update_post_meta($post_id,'vxc_infusionsoft_meta',''); 
  ...
  ?>
  <div class="alert_danger"><?php echo sprintf(__('Infusionsoft Settings are not Valid. Go to %sSettings%s','wp-infusionsoft-woocommerce'),'<a href="'.$link.'">','</a>')?></div>
  <?php
  return;
  }
  ...
  ?>
  <div class="alert_danger"><?php esc_html_e('Please Select Object','wp-infusionsoft-woocommerce')?></div>
  <?php
  return;
  }
...
$meta= is_array($meta) ? $meta : array(); 
 ...
 $fields=$this->post('fields',$feed); 
  ...
  $fields=$fields == "" ? __("No Fields Found",'wp-infusionsoft-woocommerce') : $fields;   
   ?>
  <div class="vx_error"><?php echo wp_kses_post($fields) ?></div>
  <?php
  return;
  }
  $sel_fields=array(""=>__("Standard Field",'wp-infusionsoft-woocommerce'),"custom"=>__("Custom Field",'wp-infusionsoft-woocommerce'),"value"=>__("Custom Value",'wp-infusionsoft-woocommerce'));
  ...
  $options_empty=$this->wc_select();
   ...
   $events=array();
     $events["submit"]=__("When user submits the order",'wp-infusionsoft-woocommerce');
 ...
 // $events["user_created"]=__("When user creates account on checkout page",'woo-zoho');
  $events["manual"]=__("Manually send order to Infusionsoft",'wp-infusionsoft-woocommerce');  
   if(self::$is_pr){
  $events['save_product']=__('Woocommerce Product Updated/Created','wp-infusionsoft-woocommerce');
  $events['save_user']=__('Wordpress User Updated/Created','wp-infusionsoft-woocommerce');
  if(class_exists('WC_Subscriptions')){
...
if(empty($this->feeds)){
  ...
  */
  ...
  $html="<option value=''>".esc_html__("Select Object",'wp-infusionsoft-woocommerce')."</option>";
  $res=array();
  ...
  $res['html']=$html;
      ...
      if(empty($modules)){
          $modules=__('No Objects Found','wp-infusionsoft-woocommerce');
      }
  $res['error']=$modules;   
  }
...
public function send_order_admin($post_id,$post_type=''){
 ...
 $action= $send_to_sf_button ? '' : 'update';
  ...
  $res=array("class"=>"error","msg"=>__("Nothing Posted to Infusionsoft",'woo-infusionsoft-crm-perks'));      
  }
...
$this->send_order_admin($id);
 ...
 $fields=$this->post('fields',$meta);
  ...
  if(empty($primary_key)){
    $primary_key= __("N/A",'wp-infusionsoft-woocommerce'); 
  }
  }
  ...
  if(empty($object)){
      $object=__("No Object",'wp-infusionsoft-woocommerce');
  }    
  }
  }
  switch ( $column ) {
  ...
  */
  ...
  $columns['vxc_object'] = __( 'Infusionsoft Object', 'wp-infusionsoft-woocommerce' );
  $columns['vxc_key'] = __( 'Primary Key', 'wp-infusionsoft-woocommerce' );
  ...
  */
  ...
  $this->screen_msg('updated',sprintf(__('Successfully Deleted %d Item(s)','wp-infusionsoft-woocommerce'),$count));  
    }
  ...
  $bulk_actions=array(""=>__('Bulk Action','wp-infusionsoft-woocommerce'),"delete"=>__('Delete','wp-infusionsoft-woocommerce'),
  'send_to_crm_bulk'=>__('Send to Infusionsoft','wp-infusionsoft-woocommerce'),'send_to_crm_bulk_force'=>__('Force Send to Infusionsoft - Ignore Filters','wp-infusionsoft-woocommerce'));
    $statuses=array(
    "1"=>__('Created','wp-infusionsoft-woocommerce'),
    "2"=>__('Updated','wp-infusionsoft-woocommerce'),
    "error"=>__('Failed','wp-infusionsoft-woocommerce'),
    "4"=>__('Filtered','wp-infusionsoft-woocommerce'),
    "5"=>__('Deleted','wp-infusionsoft-woocommerce')
    );
    $menu_links=array(
    array("title"=>__("Infusionsoft Settings",'wp-infusionsoft-woocommerce'),
    "link"=>$settings_link
    ),
    array("title"=>__("Infusionsoft Feeds",'wp-infusionsoft-woocommerce'),
    "link"=>$feeds_link
    ) );
...
include_once(self::$path."templates/log-entries.php");
 ...
 if($objects == ''){
  ...
  $desc.=" Web2".$row['object'];
   ...
   $desc=sprintf(__('%s filtered','wp-infusionsoft-woocommerce'),$row['object']);   
  }else   if($row['status'] == 5){
   $desc=sprintf(__('%s deleted','wp-infusionsoft-woocommerce'),$row['object']);  
  }else{
  $desc.=$row['object'];
  }
  }else{
  ...
  $title=__("Failed",'wp-infusionsoft-woocommerce');   
  if( $row['status'] == 1){
  $title=__("Created",'wp-infusionsoft-woocommerce');   
  }else if($row['status'] == 2){
  $title=__("Updated",'wp-infusionsoft-woocommerce');   
  }else if($row['status'] == 4){
  $title=__("Filtered",'wp-infusionsoft-woocommerce');   
  }else if($row['status'] == 5){
  $title=__("Deleted",'wp-infusionsoft-woocommerce');   
  }
  $row['_crm_id']= $crm_id;
  $row['a_link']=$link;
  $row['desc']=$desc;
...
$info['valid_token']= !empty($info['class']) && $info['class'] == 'updated' ? 'true' : 'false';
  ...
  if(!empty($info['valid_token'])  && $info['valid_token'] == 'true') { 
  $msg=__( 'Successfully Connected to Infusionsoft','wp-infusionsoft-woocommerce' );
     if(isset($info['_time'])){
 ...
 // ,'save_product'=>'Product Created/Updated'
  ...
  ?>
  <div class="alert_danger"><?php esc_html_e('Please Select a Infusionsoft Account','wp-infusionsoft-woocommerce')?></div>
  <?php
  die();
  }
  $this->post_id=$id=$this->post('id');
...
$data=$info['data'];
  ...
  $objects=$this->get_objects($api_type,$info); 
if(!is_array($feed)){ $feed=array(); }
  ...
  $modules=array(""=>__("Select Object",'wp-infusionsoft-woocommerce'));
  if(isset($objects) && is_array($objects)){
  if(!self::$is_pr && isset($objects['Order'])){
    unset($objects['Order']);  
  }
...
public function settings_tab(){
  ...
  $msg=__('You do not have permissions to uninstall','wp-infusionsoft-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
...
include_once(self::$path."templates/uninstall.php");
 ...
 $id=$this->post('id');
  ...
  $msg=__('Connection to Infusionsoft is Working','wp-infusionsoft-woocommerce');
  
  if($conn_class != "updated" ){
      $msg=__('Connection to Infusionsoft is NOT Working','wp-infusionsoft-woocommerce');  
  }
  $title=__('Test Connection: ','wp-infusionsoft-woocommerce');
...
include_once(self::$path."templates/settings.php");
     ...
     $msg=__('First Add your Infusionsoft account then Create Infusionsoft feed.','wp-infusionsoft-woocommerce');
     $title=__('Infusionsoft WooCommerce Setup: ','wp-infusionsoft-woocommerce');
    $this->screen_msg('updated','<b>'.$title.'</b>'.$msg); 
//echo '<div class="updated below-h2" style=""><p><b>'.$title.'</b>'.$msg.'</p></div>';
...
$res=$this->del_account($id);
 if($res){
       $msg=__('Account Deleted Successfully','wp-infusionsoft-woocommerce');
  $msg_arr=array('msg'=>$msg,'class'=>'updated');   
 }else{
       $msg=__('Error While Removing Account','wp-infusionsoft-woocommerce');
...
die();
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','wp-infusionsoft-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
  $id=$this->post('id');
...
$info=$api->handle_code();
  ...
  self::$tooltips = array(
  'sel_object' =>  __('Select the Infusionsoft object you would like to add your contacts to.', 'wp-infusionsoft-woocommerce'),
  'map_fields' =>  __('Associate your Infusionsoft fields to the appropriate order form fields by selecting.', 'wp-infusionsoft-woocommerce'),
  'optin_condition' =>__('When custom filter is enabled, orders will only be exported to Infusionsoft when all conditions match. When disabled all orders will be exported.', 'wp-infusionsoft-woocommerce'),
  'manual_export' => __('Select which WooCommerce event will automatically export the orders into Infusionsoft. To manually send an order into Infusionsoft, go to Orders, select the order, and click on the "Send to Infusionsoft" button.','wp-infusionsoft-woocommerce'),
  'vx_oauth' => __('OAuth 2.0 is a industry-standard protocol for authorization', 'wp-infusionsoft-woocommerce'),
 
  'vx_api' => __('Get Infusionsoft API key from Account Menu -> Integrations -> Get your API Key', 'wp-infusionsoft-woocommerce'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default Infusionsoft App.','wp-infusionsoft-woocommerce'),
  
  'vx_disable_logs'=>__('When an order is sent to Infusionsoft we store that order information in the database and show it in the Infusionsoft Log. Check this box if you do not want to save the exported order information in the logs.','wp-infusionsoft-woocommerce'),
  
  'vx_pro_desc'=>__('Description of Order Item.','wp-infusionsoft-woocommerce'),
  'vx_line_items'=>__('Create an Order item for each Woocommrce Cart item.','wp-infusionsoft-woocommerce'),
  

   'vx_assign_contact'=>__('Enable this option if you want to assign a Contact this object.','wp-infusionsoft-woocommerce'),
   'vx_assign_company'=>__('Enable this option if you want to assign a Company this object.','wp-infusionsoft-woocommerce'),
  
   'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','wp-infusionsoft-woocommerce'),
   'vx_sel_company'=>__('Select Contact feed. Company created by this feed will be assigned to this object','wp-infusionsoft-woocommerce'),
   
      'vx_tag_check'=>__('Enable this option if you want to assign a tag to this object.','wp-infusionsoft-woocommerce'),
   'vx_tags'=>__('Get Tags list from Infusionsoft','wp-infusionsoft-woocommerce'),
    'vx_sel_tag'=>__('Select a tag.','wp-infusionsoft-woocommerce'),
 
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','wp-infusionsoft-woocommerce'),
   'vx_owners'=>__('Get Users list from Infusionsoft','wp-infusionsoft-woocommerce'),
    'vx_sel_owner'=>__('Select a user as a owner of this object','wp-infusionsoft-woocommerce'),
    
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Infusionsoft Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Infusionsoft Object selected in the feed.','wp-infusionsoft-woocommerce'),
  
   'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note','wp-infusionsoft-woocommerce'),
   'vx_note_fields'=>__('Select fields which you want to send as a note','wp-infusionsoft-woocommerce'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry','wp-infusionsoft-woocommerce')
...
$fields=array("fields"=>$this->post('fields',$meta_post));
 ...
 if(!empty($info['meta']) ){
  ...
  case"refresh_users": 
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','wp-infusionsoft-woocommerce'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
  ...
  $res['data']=$data;   
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['vx_tags_sel']=$this->gen_select($camps,$status_sel,__('Select List','wp-infusionsoft-woocommerce'));
    $meta['tags']=$camps;  
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','wp-infusionsoft-woocommerce');
    }
}
  die(json_encode($res));    
  }
 ...
 // $crm['custom_app']=$this->post('custom_app',$crm_p);
  ...
  if(!$valid_email){
      $this->screen_msg('error',__('Invalid Email(s)','wp-infusionsoft-woocommerce'));
  }
  ...
  $msg="";
  if(empty($app_id)){
  $msg=__('Please Enter APP ID','wp-infusionsoft-woocommerce');    
  }
  if(empty($app_secret)){
  $msg=__('Please Enter APP Secret','wp-infusionsoft-woocommerce');    
  }
    if(empty($app_url)){
  $msg=__('Please Enter APP URL','wp-infusionsoft-woocommerce');    
  }
    if(!empty($msg)){
